...
export const Example = ({ i18n, data, onUpdate }: DTExampleProps) => {
  ...
  return <Dropdown value={data.example} options={options} onChange={onChange} />;
};
...
const NamesDialog = ({
  ...
  React.useEffect(() => {
    ...
    const options = countryList
      .filter((countryName: CountryType) => countryPluginsWithNames.indexOf(countryName) !== -1)
    ...
    <Dialog onClose={onClose} open={visible}>
      <div style={{ width: 500 }}>
        ...
        <DialogContent dividers>
          <div>{i18n.customizeSourceDesc}</div>
