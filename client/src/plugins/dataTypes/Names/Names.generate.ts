...
export const getRandomGender = (utils: WorkerUtils): string =>
...
export const generate = (data: any, utils: WorkerUtils): DTGenerateResult => {
	...
	if (source === NamesSource.countries) {
		const randomCountry: string =
			utils.randomUtils.getRandomArrayValue(selectedCountries);
	...
	while (/MaleName/.test(output)) {
		...
		output = output.replace(
			/MaleName/,
			utils.randomUtils.getRandomArrayValue(maleNamesSource)
		);
	}
	...
	while (/FemaleName/.test(output)) {
		...
		output = output.replace(
			/FemaleName/,
			utils.randomUtils.getRandomArrayValue(femaleNamesSource)
		);
	}
	...
	while (/Name/.test(output)) {
		...
		const selectedSource =
			gender === 'male' ? maleNamesSource : femaleNamesSource;
		output = output.replace(
			/Name/,
			utils.randomUtils.getRandomArrayValue(selectedSource)
		);
	}
	...
	while (/Surname/.test(output)) {
		output = output.replace(
			/Surname/,
			utils.randomUtils.getRandomArrayValue(lastNamesSource)
		);
	}
	while (/Initial/.test(output)) {
		output = output.replace(
			/Initial/,
			utils.randomUtils.getRandomCharInString(letters)
		);
	}
