...
import { dataTypes, blacklistedDataTypeFolders } from '../../_plugins';
...
let lastLocale;
let cachedSortedGroupedDataTypes;
export const getSortedGroupedDataTypes = () => {
    ...
    lastLocale = locale;
    cachedSortedGroupedDataTypes = C.DATA_TYPE_GROUPS.map((group) => {
        const options = Object.keys(dataTypes)
            .filter((dataType) => dataTypes[dataType].fieldGroup === group)
        ...
        const sortedOptions = options.map(({ dataType }) => ({
            value: dataType,
...
export const DefaultHelpComponent = ({ i18n }) => <p dangerouslySetInnerHTML={{ __html: i18n.DESC }}/>;
...
export const getDataType = (dataType) => {
    ...
    const { getMetadata, rowStateReducer } = loadedDataTypes[dataType];
    return {
        isLoaded: true,
        Options,
        Help,
...
export function RecursiveErrorException(remaining) {
    // @ts-ignore-line
 ...
 */
export const getProcessBatches = (dataTypes) => {
    ...
    while (dataTypesToProcess.length > 0) {
        ...
        dataTypesToProcess.forEach((dataType) => {
            ...
            const allDependenciesPositioned = !dataTypes[dataType].dependencies || dataTypes[dataType].dependencies.every((dependency) => !!processBatches[dependency]);
            if (allDependenciesPositioned) {
                resolvedDataTypes.push(dataType);
            }
        });
        if (resolvedDataTypes.length) {
            resolvedDataTypes.forEach((dataType) => {
...
export const getAffectedDataTypes = (dataTypes) => {
    const affectedDataTypes = {};
    Object.keys(dataTypes).forEach((dataType) => {
        ...
        if (dataTypes[dataType].dependencies) {
            dataTypes[dataType].dependencies.forEach((dep) => {
...
export const requestDataTypeBundle = (dataType) => {
...
// const noValue: any = {};
export const getCustomProps = (customProps, state) => {
    const values = {};
    if (customProps) {
