...
describe('getRandomNum', () => {
	...
	it('generates numbers in expected range #2', () => {
		...
		for (let i = 0; i < 50; i++) {
			expect(
				expected.indexOf(randomUtils.getRandomNum(5, 7)) !== -1
			).toBeTruthy();
		}
	});
	...
	it('generates numbers in expected range #3', () => {
		...
		for (let i = 0; i < 50; i++) {
			expect(
				expected.indexOf(randomUtils.getRandomNum(0, 1)) !== -1
			).toBeTruthy();
		}
	});
	...
	it('generates numbers in expected range #4', () => {
		...
		for (let i = 0; i < 50; i++) {
			expect(
				expected.indexOf(randomUtils.getRandomNum(-2, 2)) !== -1
			).toBeTruthy();
		}
	});
	...
	it('generates numbers when number range entirely negative', () => {
		...
		for (let i = 0; i < 50; i++) {
			expect(
				expected.indexOf(randomUtils.getRandomNum(-4, -1)) !== -1
			).toBeTruthy();
		}
	});
...
describe('generateRandomTextStr', () => {
	...
	it('generates multiple words starting from the first word', () => {
		expect(randomUtils.generateRandomTextStr(words, true, 2)).toEqual(
			'one two'
		);
		expect(randomUtils.generateRandomTextStr(words, true, 3)).toEqual(
			'one two three'
		);
	});
	...
	it('generates two random words', () => {
		expect(randomUtils.generateRandomTextStr(words, false, 2)).toMatch(
			/[a-z]+\s[a-z]+/
		);
	});
	...
	it('generates the same string every time when min num of words is the total number of words', () => {
		for (let i = 0; i < 10; i++) {
			expect(
				randomUtils.generateRandomTextStr(['a', 'b', 'c'], false, 3)
			).toEqual('a b c');
		}
	});
});
...
describe('generateRandomAlphanumericStr', () => {
	...
	it('special characters are converted as expected', () => {
		...
		for (let i = 0; i < 50; i++) {
			...
			expect(
				randomUtils.lowercaseConsonants.indexOf(generated[1]) !== -1
			).toBeTruthy();

			// A = any case consonant
			expect(
				randomUtils.consonants
					.concat(randomUtils.lowercaseConsonants)
					.indexOf(generated[2]) !== -1
			).toBeTruthy();
			...
			// v = uppercase vowel
			expect(
				randomUtils.lowercaseVowels.indexOf(generated[4]) !== -1
			).toBeTruthy();

			// F = any case vowel
			expect(
				randomUtils.vowels
					.concat(randomUtils.lowercaseVowels)
					.indexOf(generated[5]) !== -1
			).toBeTruthy();
			...
			// l = lowercase letter
			expect(
				randomUtils.lowercaseLetters.indexOf(generated[7]) !== -1
			).toBeTruthy();

			// D = any case letter
			expect(
				randomUtils.letters
					.concat(randomUtils.lowercaseLetters)
					.indexOf(generated[8]) !== -1
			).toBeTruthy();

			// X = 1-9
			expect(
				['1', '2', '3', '4', '5', '6', '7', '8', '9'].indexOf(generated[9]) !==
					-1
			).toBeTruthy();
...
describe('getRandomSubset', () => {
	...
	it('returns the same content when passing a full array', () => {
		expect(randomUtils.getRandomSubset([1, 2, 3], 3).sort()).toEqual(
			[1, 2, 3].sort()
		);
	});
});
...
describe('generatePlaceholderStr', () => {
	it('generates string using custom placeholders', () => {
		...
		expect(
			randomUtils.generatePlaceholderStr('ABC', defaultPlaceholders)
		).toEqual('123');
	});
});
