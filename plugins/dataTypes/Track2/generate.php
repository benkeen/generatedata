...
function track2_generate_item($row, $str, $existing_row_data){
	
	
	$track2 = track2_generate_track_number();
 ...
 * @return string
 */
function track2_get_export_type_info($export_type, $options)
{
  $info = "";
  switch ($export_type)
  {
...
//---------to create 26 random numbers-------------------
 function track2_rand_str($length = 26, $chars = '1234567890')
								{
 ...
 function track2_completed_number($prefix, $length) {
 ...
 function track2_credit_card_number($prefixList, $length, $howMany) {
 ...
 function track2_generate_track_number(){
	...
	$characters = array("A","B","C","D","E","F","G","H","J","K","L","M","N","P","Q","R","S","T","U","V","W","X","Y","Z");
		...
		$num1 = track2_rand_str();
		$num2 = track2_rand_str();
		...
		$track2 = ";$rand_ccnum[$get_rand_ccnum]=$calender$service_code$num2?";
		$random_chars =NULL;
		return $track2;
