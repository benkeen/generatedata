...
export const getRoutes = () => {
    ...
    customRoutes.forEach(({ path, component }) => {
        let found = false;
...
export const getUnlocalizedGeneratorRoute = () => process.env.GD_GENERATOR_PATH || '';
export const getGeneratorPageRoute = (locale) => {
...
export const isGeneratorPage = (currentPage, locale) => {
...
let customHeaderLinkGetter;
// allows external code to override the header links
export const registerCustomHeaderLinksGetter = (getter) => {
    customHeaderLinkGetter = getter;
};
export const getHeaderLinks = (isLoggedIn, accountType) => {
...
export const updateBodyClass = (store, pathname) => {
    let pageId = pathname.replace(/\//g, '');
    const rootLocale = env.availableLocales.indexOf(pageId);
