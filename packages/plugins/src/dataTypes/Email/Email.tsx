      ...
      <div className={classNames.fieldsRow}>
        ...
        <div className={classNames.fieldRow}>
          ...
          <Dropdown
            ...
            options={[{ value: '', label: i18n.optional }, ...rowOptions]}
          />
        </div>
      </div>
    );
...
export const Options = ({ i18n, coreI18n, id, data, onUpdate, nameRows }: DTOptionsProps) => {
  ...
  const rowOptions = nameRows.map(({ id: currentId, title, index }: any) => ({
    value: currentId,
...
export const rowStateReducer = (state: EmailState): GenerationOptionsType => {
  ...
  return {
    ...safeData,
    domains: safeData.domains
      .split(',')
      .filter((i) => !!i)
      .map((i) => i.replace(/[^0-9a-zA-Z]/g, '')),
    domainSuffixes: safeData.domainSuffixes
      .split(',')
      .filter((i) => !!i)
      .map((i) => i.replace(/[^0-9a-zA-Z]/g, ''))
  };
