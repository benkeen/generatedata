...
describe('getRandomEmailPrefix', () => {
  ...
  it('generates an email address with a single word prefix (word before the @)', () => {
    sinon
      .stub(utils.randomUtils, 'getRandomNum')
      .onCall(0)
      .returns(1) // number of prefixed words
      .onCall(1)
      .returns(0); // prefix word offset
  ...
  it('multiple prefix words are separated by dots #1', () => {
    sinon
      .stub(utils.randomUtils, 'getRandomNum')
      .onCall(0)
      .returns(2) // number of prefixed words
      .onCall(1)
      .returns(3); // prefix word offset
  ...
  it('multiple prefix words are separated by dots #3', () => {
    sinon
      .stub(utils.randomUtils, 'getRandomNum')
      .onCall(0)
      .returns(3) // number of prefixed words
      .onCall(1)
      .returns(2); // prefix word offset
  ...
  it('punctuation in words (.,;:) are removed', () => {
    sinon
      .stub(utils.randomUtils, 'getRandomNum')
      .onCall(0)
      .returns(4) // number of prefixed words
      .onCall(1)
      .returns(0); // prefix word offset
  ...
  it('picks a random suffix', () => {
    sinon
      ...
      .returns(0) // prefix word offset
      .onCall(2)
      .returns(1) // number of domain words
      .onCall(3)
      .returns(2); // domain word offset
